...
document.addEventListener("DOMContentLoaded", function () {
    gameArea = document.getElementById("tile-container");
    gameArea.style.width = dim * tileWidth + "px";
    gameArea.style.height = dim * tileWidth + "px";
    document.getElementById("game-container").style.width = dim * tileWidth + "px";
...
function Othello(userColor) {
    this.computer = userColor === "black" ? 1 : -1;
    this.turn = -1;
    this.whiteScore = 0;
    this.blackScore = 0;
    ...
    for (var i = 2; i < 7; i += 4) {
        for (var j = 2; j < 7; j += 4) {
            ...
            dot.setAttribute('class', 'note');
            dot.style.top = i * tileWidth - 5 + "px";
            dot.style.left = j * tileWidth - 5 + "px";
...
Othello.prototype.gameOver = function () {
    if (this.blackScore > this.whiteScore) {
        checkHighScore(this.blackScore);
        document.getElementById("result").textContent = "Black Wins!";
    }else if (this.blackScore < this.whiteScore) {
        checkHighScore(this.whiteScore);
        document.getElementById("result").textContent = "White Wins!";
    } else {
...
Othello.prototype.setColor = function (i, j, flip) {
    ...
    document.getElementById("score-black").textContent = this.blackScore;
    document.getElementById("score-white").textContent = this.whiteScore;
}
...
Tile.prototype.setTileBlack = function () {
    if (this.value == 1) this.view.classList.remove("white-piece")
    this.value = -1;
    this.view.classList.add("black-piece");
}

Tile.prototype.setTileWhite = function () {
    if (this.value == -1) this.view.classList.remove("black-piece")
    this.value = 1;
    this.view.classList.add("white-piece");
}

function createTile(i, j) {
    var tileBg = document.createElement('div');
    tileBg.setAttribute('class', 'tile-background');
    tileBg.setAttribute('i', i);
    tileBg.setAttribute('j', j);


    tileBg.style.top = i * tileWidth + "px";
    tileBg.style.left = j * tileWidth + "px";

    tileBg.addEventListener('click', function () {
        if (game.isValidMove(this.getAttribute("i"), this.getAttribute("j")))
            game.makeMove(this.getAttribute("i"), this.getAttribute("j"));
    });

    tileBg.addEventListener('mouseenter', function () {
        if (game.isValidMove(this.getAttribute("i"), this.getAttribute("j")))
            tileBg.style.backgroundColor = "#62e984";
    });

    tileBg.addEventListener('mouseleave', function () {
        if (game.isValidMove(this.getAttribute("i"), this.getAttribute("j")))
            tileBg.style.backgroundColor = "#42b964";
    });
...
function checkHighScore(gameScore) {
    ...
    if(!score || (gameScore > parseInt(score))) {
        localStorage.setItem("score", gameScore);
        document.getElementById("highscore").textContent = gameScore;
